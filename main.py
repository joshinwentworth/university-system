...
class LoginFrame(tk.Frame):
    ...
    def login(self, event=None):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT PASSWORD, ROLE, EMAIL FROM LOGIN ")
        row = cursor.fetchone()
        conn.close()
        ...
        if (row and row[0] == password)and(row[2] == username):
            ...
            if self.master.user_role == "Admin":
                ...
                self.master.switch_frame(PlayerHubFrame)
        else:
...
class PlayerHubFrame(ttk.Frame):
    target_state = 'zoomed'
    ...
    def create_gui(self):
        ...
        ttk.Label(welcome_frame, text="Select a game below", font=("Arial", 12)).pack()
        ...
        # Games Grid
        games_frame = ttk.LabelFrame(self, text="Games", padding=15)
        games_frame.pack(fill=tk.BOTH, expand=True, pady=10)

        btn_row = 0
        btn_col = 0
        
        for game_name, img_path in image_paths.items():
            try:
                ...
                btn = ttk.Button(
                    ...
                    image=photo,
                    compound="top",
                    command=lambda g=game_name: self.open_game(g)
                )
                btn.image = photo 
                btn.grid(row=btn_row, column=btn_col, padx=15, pady=12, ipadx=10, ipady=8)

                # Button Grid: 3 row x 2 column
                btn_col += 1
                if btn_col > 1:
                    btn_col = 0
                    btn_row += 1
                ...
                print(f"Warning: Icon missing for {game_name}: {img_path}")
                # Fallback text button if image fails
                ttk.Button(games_frame, text=game_name, command=lambda g=game_name: self.open_game(g)).grid(row=btn_row, column=btn_col, padx=15, pady=12)
